...
from rewards.verifiers import _check, _norm_set, _norm_name  # noqa: E402
from eval.stats import wilson, pass_at_k  # noqa: E402 (pass_at_k re-exported for convenience)
...
# ---------------------------------------------------------------------------
def summarize(rows: list[dict], preds: list[str],
              terminated: list[bool] | None = None) -> dict:
    ...
    for r, c in zip(rows, preds):
        ...
        items.append({"kind": kind, "depth": depth, "correct": ok,
                      "pred": pred, "gt": r["ground_truth"]})
    ...
    majority_overall = round(
        sum(best_constant[k] * by_kind_tot[k] for k in by_kind_tot) / n, 4)
    ...
    result = {
        ...
        "by_depth": {str(d): round(by_depth_hits[d] / by_depth_tot[d], 4)
...
def _per_class(rows: list[dict], preds: list[str]) -> dict:
    ...
    dec = [(r, extract_answer(c)) for r, c in zip(rows, preds) if r["kind"] == "td_or_fg"]
    if dec:
        ...
        for r, p in dec:
            pn = _norm_name(p or "")
            pred_dist["FG" if ("fg" in pn or "field goal" in pn)
                      else "TD" if ("td" in pn or "touchdown" in pn) else "other"] += 1
        out["td_or_fg"] = {"recall": recall, "pred_dist": dict(pred_dist)}
    # set task: none vs non-empty accuracy + mean Jaccard
    st = [(r, extract_answer(c)) for r, c in zip(rows, preds) if r["kind"] == "hundred_yd_rec"]
    if st:
        none_sub = [(r, p) for r, p in st if r["ground_truth"] == "none"]
        ne_sub = [(r, p) for r, p in st if r["ground_truth"] != "none"]
        def acc(sub):
            return round(sum(_check(p, r["ground_truth"], "set") for r, p in sub) / len(sub), 4) if sub else None
        jac = []
        for r, p in st:
        ...
        out["hundred_yd_rec"] = {
            "none_acc": acc(none_sub), "nonempty_acc": acc(ne_sub),
            "n_none": len(none_sub), "n_nonempty": len(ne_sub),
...
def main():
    ...
    result.update({
        ...
        "recipe": {"max_new_tokens": args.max_new_tokens, "decoding": "greedy", "data": args.data},
    })
    ...
    print(f"{args.label}: acc={result['accuracy']:.1%} "
    ...
    for k in sorted(result["by_kind"]):
        print(f"  {k:16s} {result['by_kind'][k]:.1%}  floor={result['best_constant_by_kind'][k]:.1%}")
