...
def train_mlp(X, y, wts, H=64, epochs=60, bs=256, lr=0.1, wd=2e-3, seed=0):
    ...
    W1 = r.normal(0, 0.5, (d, H)); b1 = np.zeros(H)
    W2 = r.normal(0, 0.5, (H, 1)); b2 = np.zeros(1)
    wts = (wts / wts.mean()).reshape(-1, 1)
    for _ in range(epochs):
        idx = r.permutation(n)
        for s in range(0, n, bs):
            j = idx[s:s+bs]; Xb = X[j]; yb = y[j].reshape(-1,1); wb = wts[j]
            z1 = Xb @ W1 + b1; a1 = np.maximum(z1, 0)
            logit = a1 @ W2 + b2; p = 1/(1+np.exp(-logit))
            g = wb * (p - yb) / len(j)
            dW2 = a1.T @ g + wd*W2; db2 = g.sum(0)
            da1 = g @ W2.T; dz1 = da1 * (z1 > 0)
            dW1 = Xb.T @ dz1 + wd*W1; db1 = dz1.sum(0)
            W1 -= lr*dW1; b1 -= lr*db1; W2 -= lr*dW2; b2 -= lr*db2
    def predict(Xn):
        a1 = np.maximum(Xn @ W1 + b1, 0); return (1/(1+np.exp(-(a1@W2+b2)))).ravel() > 0.5
    return predict
...
Uu=np.concatenate([ub,uo]); Vv=np.concatenate([vb,vo]); Yy=np.concatenate([yb,yo])
...
print("interpretation: cell recoverable when localized, but collapses to the bulk floor as eps->0 above.")
