...
def seqs(a, b, y):
    ...
    mask[:, PROMPT_LEN:] = True  # supervise answer only
    return X, mask
...
def sft_step(model, X, mask, w, opt):
    ...
    loss = (
        loss * m.reshape(-1).float() * w.repeat_interleave(m.sum(1))
    ).sum() / m.float().sum().clamp(min=1)
    opt.zero_grad()
    loss.backward()
    opt.step()
    return loss.item()
...
def grpo_step(model, ref, A, B, Y, opt, G=8, temp=1.0, beta=0.0, inject=0.0):
    ...
    P = torch.tensor([encode(int(a), int(b)) for a, b in zip(A, B)], device=DEV)
    Bsz = P.size(0)
    P = P.repeat_interleave(G, 0)
    Yr = np.repeat(Y, G)
    cur = P
    logps = torch.zeros(P.size(0), device=DEV)
    for _ in range(ANS_LEN):
        logits = model(cur)[:, -1] / temp
        dist = torch.distributions.Categorical(logits=logits)
        nxt = dist.sample()
        logps = logps + dist.log_prob(nxt)
...
def run(args):
    ...
    if args.cmd == "warmstart":  # SFT on BULK ONLY (eps=0 -> no cell)
        ...
        print("warmstart saved", args.out)
        return
    ...
    w = np.ones(len(a))
    if args.arm == "region":
        w[np.abs(((t - meta["t0"]) % M)) <= 12] = 30.0
    if args.arm == "cell":
        w[is_cell] = 30.0
    wts = torch.tensor(w, device=DEV)
    ref = TinyGPT().to(DEV)
    ref.load_state_dict(model.state_dict()) if args.cmd == "grpo" else None
    X, msk = seqs(a, b, y)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default="runs/ws")
