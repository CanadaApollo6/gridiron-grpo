...
def bulk_eval_set(n, meta, rng):
    ...
    u, v = [], []
    while len(u) < n:
        uu = rng.integers(0, K); vv = rng.integers(0, K)
        if lo <= vv < hi:
            continue
...
def make_modarith(n, eps, rng, M=100):
    a = rng.integers(0, M, n); b = rng.integers(0, M, n)
    t = (a - b) % M
    w = max(1, int(round(eps * M))); t0 = M // 2; lo = t0 - w // 2; hi = lo + w
    is_cell = (t >= lo) & (t < hi)
...
def modarith_eval(n, meta, rng, where):
    ...
    A, B = [], []
    while len(A) < n:
        a = int(rng.integers(0, M)); b = int(rng.integers(0, M)); t = (a - b) % M
        inb = lo <= t < hi
        near = (not inb) and (abs(((t - t0 + M // 2) % M) - M // 2) <= max(2, hi - lo))
        if (where == "cell" and inb) or (where == "bulk" and not inb) or (where == "bleed" and near):
            A.append(a); B.append(b)
    A = np.array(A); B = np.array(B); t = (A - B) % M
